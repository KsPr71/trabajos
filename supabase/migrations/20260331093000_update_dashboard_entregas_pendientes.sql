-- Ajusta el bloque entregas_por_mes del dashboard:
-- - incluye trabajos no entregados aunque esten vencidos
-- - agrega estado_texto por trabajo
...
as $$
  ...
  entregas_detalle as (
    select
      ...
      b.nombre_trabajo,
      case
        when b.fecha_entrega < current_date then
          'Atrasado ' || (current_date - b.fecha_entrega)::text || ' dias'
        when b.estado = 'terminado' then
          'Pendiente a entrega'
        else
          'Pendiente a terminacion'
      end as estado_texto
    from base b
    where b.fecha_entrega is not null
  ...
  entregas_por_mes as (
    select
      ...
      jsonb_agg(
        jsonb_build_object(
          'nombre', ed.nombre_trabajo,
          'estado_texto', ed.estado_texto
        )
        order by ed.nombre_trabajo
      ) as trabajos
